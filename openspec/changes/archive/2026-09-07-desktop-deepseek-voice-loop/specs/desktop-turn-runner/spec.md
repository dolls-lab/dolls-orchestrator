...
The runner SHALL support an offline profile for deterministic verification, a local-no-api profile that wires MLX Whisper to a fixed LLM and macOS speech synthesis, and a local-voice profile that wires MLX Whisper, the DeepSeek placeholder, and macOS speech synthesis through the same contracts.
...
The application SHALL load non-secret defaults and environment overrides into one validated configuration object, and secrets SHALL only be accepted through environment variables.
...
- **WHEN** a timeout, context limit, or output-token limit is outside its allowed range
